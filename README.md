# I3E_ASG1

# Escape the Haunted School

## GitHub Repository

https://github.com/sheryn7/I3E_ASG1

---

## Overview

**Escape the Haunted School** is a first-person horror puzzle game where players must explore an abandoned school, collect important items, unlock restricted areas, survive hazards and obtain the Master Key in order to escape.

---

## How to Run the Application

1. Extract the ZIP file
2. Launch the executable (`.exe`) file
3. Wait for the game to load
4. Click **Start** to begin playing
5. Follow the on-screen instructions and interact with objects to progress through the game

---

## Controls

| Key | Function |
|------|----------|
| **W** | Move Forward |
| **A** | Move Left |
| **S** | Move Backward |
| **D** | Move Right |
| **Mouse** | Look Around |
| **E** | Interact / Open Doors / Collect Items |
| **ESC** | Exit Application |

---

## Gameplay Objective

The objective of the game is to escape the haunted school.

To complete the game, players must:

1. Collect all **4 collectibles**.
2. Unlock the **Staff Room**.
3. Find and collect the **Master Key**.
4. Use the Master Key to unlock the **Escape Door**.
5. Escape the school successfully.

---

## Puzzle Walkthrough / Answer Key

### Collectibles

The player must collect the following items found in the classroom:

- Student ID (at the back, close to the chair)
- Book (on a table)
- File (under a table)
- Pencil Case (in the trash bin)

### Staff Room

- The Staff Room remains locked until all **4 collectibles** have been collected.
- Once all collectibles are obtained, the Staff Room will unlock.

### Master Key

- The Master Key is hidden inside a locker within the Staff Room.
- Interact with the correct locker to reveal and collect the key.

### Escape Door

- After obtaining the Master Key, proceed to the Escape Door.
- Press **E** to escape and complete the game.

---

## Hazards

### Monster Jumpscare

Entering the Staff Room trigger area will activate a monster jumpscare.

Effects:
- Player health immediately drops to **0**.
- A death screen is displayed.
- The player respawns at the corridor checkpoint.
- The monster disappears after the respawn.

### Monster Residue (Puddle Hazard)

Stepping into the puddle hazard will:

- Continuously decrease player health.
- Display a warning message.
- Stop dealing damage once the player leaves the puddle.

---

## Platform Requirements

### Operating System

- Windows 10
- Windows 11

### Minimum Hardware Requirements

- Intel Core i3 Processor
- 4 GB RAM
- DirectX 11 Compatible Graphics Card

### Recommended Hardware Requirements

- Intel Core i5 Processor or better
- 8 GB RAM
- Dedicated Graphics Card

### Resolution

- Recommended: **1920 × 1080**

### Unity Version

- Unity 6.3 LTS (6000.3.13f1)

---

## Known Limitations / Bugs

1. The monster jumpscare can only be triggered once.
2. Health resets to full after respawning from the monster encounter.
3. The game currently contains only one playable level.
4. No save/load system is implemented.

---

## Features Implemented

### Player Features

- First-person movement
- Health system
- Respawn system
- Interaction system

### Environment Features

- Interactive doors
- Locked Staff Room
- Escape Door
- Collectible system
- UI collectible counter
- Health display
- Hazard damage system
- Jumpscare event
- Ending screen

---

## References and Credits

### Custom Models
- Tables and Chairs
- Board
- 4 Collectibles (Student ID, Book, File, Pencil Case)
- Teacher Table
- Lights
- Structure of School Building (using probuilder)

### Assets from Unity Asset Store

- Monster Model – Monster Mutant 7 by Panchenko Lyudmila
- Locker Models – Locker by VIS Games
- Door Model – Classic Interior Door Pack 1 by Jan Fidler
- Key Model – Simple Keys by 3Digitalis

### Textures and Materials

- Floor Textures – Tiles133D by AmbientCG
- Wall Textures – Plaster006 by AmbientCG
- Wood Textures – wood076 by AmbientCG

### Audio

- PickUp SFX by <a href="https://pixabay.com/users/freesound_community-46691455/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=37089">freesound_community</a> from <a href="https://pixabay.com/sound-effects//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=37089">Pixabay</a>
- Open Door SFX by <a href="https://pixabay.com/users/dragon-studio-38165424/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=454245">DRAGON-STUDIO</a> from <a href="https://pixabay.com/sound-effects//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=454245">Pixabay</a>
- Close Door SFX by <a href="https://pixabay.com/users/freesound_community-46691455/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=79144">freesound_community</a> from <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=79144">Pixabay</a>
- PickUp Key by <a href="https://pixabay.com/users/litupsubway-55283183/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=522219">litupsubway</a> from <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=522219">Pixabay</a>
- Jumpscare by <a href="https://pixabay.com/users/dubnium-46834607/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=258269">d d</a> from <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=258269">Pixabay</a>
- Locker Open by <a href="https://pixabay.com/users/freesound_community-46691455/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=101487">freesound_community</a> from <a href="https://pixabay.com/sound-effects//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=101487">Pixabay</a>
- Damage Taken by <a href="https://pixabay.com/users/fxprosound-41783058/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=248606">Robert Gacek</a> from <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=248606">Pixabay</a>

### Development Assistance

- OpenAI ChatGPT was used as a learning and development aid for:
- Unity scripting guidance
- C# debugging and troubleshooting
- XML documentation comments
- README documentation preparation
- General game development consultation

### Software Used

- Unity 6.3 LTS
- Visual Studio
- Maya

---

## Author

**Sheryn Batrisyia**

Created for **Interactive 3D Experience Assignment 1**.